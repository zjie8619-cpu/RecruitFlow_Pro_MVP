...
class RecruitPipeline:
    ...
    def generate_jd(self, job: str, must_have: str = "", nice_to_have: str = "", exclude_keywords: str = "", use_ai: bool = None):
        ...
        # 判断是否使用AI
        if use_ai is None:
            use_ai = self.cfg.get("llm_provider") in ["openai", "claude", "siliconflow"]
        
        # 如果启用AI，尝试使用AI生成
        if use_ai:
            try:
                provider = self.cfg.get("llm_provider", "openai")
...
2）基于 CRM 跟进线索，推进试听课与签约；
3）与教研/运营协作，保障服务满意度与续费；
4）数据驱动复盘，持续优化转化话术。

【任职要求】

必备：{must or "沟通力强；结果导向；自驱"}；

加分：{nice or "在线教育经验；数据意识；熟悉CRM"}；

排除：{exclude or "短期实习；频繁跳槽"}；
"""
        jd_short = f"{job}｜高转化话术库 + 稳定客源 + 成长路径。"
    ...
    def ingest_resumes_df(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            cur.execute("""INSERT INTO resume (id,name,email,phone,edu,companies,years,skills,projects,text_raw,source,created_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""", (
                str(uuid.uuid4()), str(row["name"]), str(row["email"]), str(row["phone"]),
                str(row["edu"]), str(row["companies"]), float(row["years"] or 0),
                str(row["skills"]), str(row["projects"]), str(row["text_raw"]), "csv", time.strftime("%Y-%m-%d %H:%M:%S")))
    ...
    def ingest_text_resume(self, txt: str):
        ...
        cur.execute("""INSERT INTO resume (id,name,email,phone,edu,companies,years,skills,projects,text_raw,source,created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""", (
            str(uuid.uuid4()), parsed.get("name",""), parsed.get("email",""), parsed.get("phone",""),
            parsed.get("edu",""), "", float(parsed.get("years") or 0.0), "", "", txt, "txt", time.strftime("%Y-%m-%d %H:%M:%S")))
    ...
    def score_all(self, job: str) -> pd.DataFrame:
        ...
        for _, row in out.iterrows():
            cur.execute("""INSERT INTO score (id,resume_id,job,score_total,skill_fit,exp_relevance,stability,growth,evidence_json,confidence,created_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)""", (
                str(uuid.uuid4()), row["id"], job, float(row["score_total"]), float(row["skill_fit"]),
                float(row["exp_relevance"]), float(row["stability"]), float(row["growth"]),
                json.dumps({"evidence": row["evidence"]}, ensure_ascii=False), float(row["confidence"]), time.strftime("%Y-%m-%d %H:%M:%S")))
    ...
    def dedup_and_rank(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        对候选人结果去重、按总分排序。
        自动识别得分列名（兼容 'score_total'、'总分'、'score' 等）
        """
        ...
        if not score_col:
            raise KeyError(f"未找到评分列，请确认评分结果包含以下任一字段: {possible_cols}")
