...
def make_invite_email(candidate: dict, job: str) -> str:
    ...
    if candidate.get("years"): hi.append(f"相关年限：{candidate['years']} 年")
    h = "；".join(hi[:2]) if hi else "匹配度良好，期待交流"
    return f"""Hi {name}，

我们是教育科技公司人力团队，关于「{job}」岗位想与您安排一次 30-45 分钟的初步交流。
初步评估亮点：{h}

若您方便，请接受随信附带的日历邀请（.ics），届时我们在企业微信/Zoom 见。
