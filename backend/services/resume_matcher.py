# ======================================================
# 🧠 ResumeAI 防幻觉与行业纠偏引擎(Final Ver.?# 作者:ChatGPT 企业级优化版
# 功能?#   - 避免 GPT 自动编?竞赛获奖""教学经验"等虚假内?#   - 根据岗位语义自动修正输出结果
#   - 支持所有行业(销?运营/教育/技?行政等)
# ======================================================
...
# 🚧 定义行业关键字(语义识别层)
JOB_DOMAINS = {
#     "销?: ["销?, "顾问", "电销", "邀?, "转化", "商务", "客户", "课程"],
#     "运营": ["运营", "推广", "策划", "新媒?, "小红?, "抖音", "公众?],
#     "教育": ["教学", "讲师", "教师", "辅导", "学员", "课程设计", "培训?],
#     "技?: ["开?, "工程?, "测试", "系统", "算法", "代码"],
#     "行政": ["行政", "人事", "档案", "绩效", "考勤"]
}
...
#     r"获得.{0,6}竞赛?, r"拥有.{0,6}教学经验", r"指导.{0,6}学生", r"辅导.{0,6}比赛",
...
#         "岗位": job_title,
#         "行业判断": industry,
#         "文本样本": clean_text_resume[:200]  # 用于日志或调试预?    }
...
#     判断候选人是否具备真实的新媒体运营经验(向后兼容)
    ...
    # 精准关键?    newmedia_keywords = [
#         "新媒?, "内容运营", "公众?, "抖音", "小红?, "知乎", "视频?,
#         "短视?, "自媒?, "社群运营", "内容策划", "平台运营"
    ]

    # 操作动词:必须同时出现这些词才算真运?    action_words = ["负责", "运营", "策划", "发布", "管理", "编辑", "搭建"]
