...
def call_siliconflow(prompt: str, model: str = "deepseek-chat", temperature: float = 0.7) -> str:
    ...
    try:
        ...
        base_url = api_keys.get("siliconflow_base_url", "https://api.siliconflow.cn/v1")
        
        if not api_key:
            raise ValueError("未在配置文件中找到 siliconflow_api_key，请检查 backend/configs/api_keys.json")
